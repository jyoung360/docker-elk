...
ADD data/txt_messages.csv /var/log/
ADD data/payments.csv /var/log/
...
ADD conf/data.html /opt/kibana/src/public/

ADD conf/supervisord.conf /etc/supervisor/conf.d/
ADD conf/messages.conf /etc/logstash/conf.d/
ADD conf/payments.conf /etc/logstash/conf.d/
ADD conf/kafka.conf /etc/logstash/conf.d/
ADD conf/elasticsearch.yml /etc/elasticsearch/
ADD conf/payment.json /etc/elasticsearch/config/templates/
ADD conf/message.json /etc/elasticsearch/config/templates/
ADD conf/kafka.json /etc/elasticsearch/config/templates/
ADD conf/logs.json /etc/elasticsearch/config/mappings/


#RUN /opt/logstash/bin/plugin install logstash-input-sqlite
